...
### 3.1. Chuẩn hóa sơ đồ luồng kiến trúc và phân vùng hệ thống (Architecture & Multi-Zone Diagrams)
* **Sơ đồ Kiến trúc Phân lớp và Phân vùng Hệ thống (Enterprise Multi-Zone Architecture):**
  * Sử dụng `flowchart TD` phân định 5 phân vùng an ninh chuẩn Viettel (`ZONE_ACCESS` → `ZONE_DMZ` → `ZONE_INT` → `ZONE_OAM` → `ZONE_DB`).
  * Các phân vùng con sử dụng `direction LR` và dàn ngang bằng `~~~`. Riêng vùng Dải Trong `ZONE_INT` dùng `direction TB` chứa các cụm dịch vụ (`SRV_CORE`, `SRV_PARTNER`, `SRV_BROKER`).
  * Đường nối liên phân vùng thể hiện giao thức bảo mật (TLS 1.3, Pin-hole Firewall Token JWT, TCP Socket CSDL, VPN LAN).
  * Directive chuẩn: `%%{init: {'flowchart': {'nodeSpacing': 14, 'rankSpacing': 24, 'padding': 8}, 'themeVariables': {'fontSize': '13px', 'fontFamily': 'Inter, Arial, sans-serif'}}}%%`.
  * Bảng màu CSS chuẩn hóa: `cClient`, `cExt`, `cDmz`, `cCore`, `cPartner`, `cKafka`, `cGov`, `cOam`, `cDb`.
  * Bắt buộc có Bảng Đặc tả Phân vùng Mạng 5 cột đi kèm làm căn cứ cấu hình hạ tầng.
* **Sơ đồ Luồng Quy trình Kiến trúc 2 Cột Song Song (`flowchart LR`):**
