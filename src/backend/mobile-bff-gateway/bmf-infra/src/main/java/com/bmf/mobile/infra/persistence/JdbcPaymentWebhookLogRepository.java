...
public class JdbcPaymentWebhookLogRepository implements PaymentWebhookLogRepository {
    ...
    private final Map<String, PaymentWebhookLog> memoryStore = new ConcurrentHashMap<>();

    @Override
    public void save(PaymentWebhookLog webhookLog) {
        memoryStore.put(webhookLog.getTraceId(), webhookLog);
        try {
            String sql = """
            ...
            log.warn("Database insert failed for webhook log (using memory store fallback): traceId={}", webhookLog.getTraceId());
        }
    }
    ...
    public Optional<PaymentWebhookLog> findByTraceId(String traceId) {
        try {
            ...
            return Optional.ofNullable(memoryStore.get(traceId));
        }
    }
}
