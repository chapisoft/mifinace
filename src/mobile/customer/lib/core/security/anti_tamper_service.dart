...
class AntiTamperService {
  ...
  static Future<DeviceSecurityStatus> auditDeviceIntegrity() async {
    ...
    // 1. Check known root / jailbreak binaries
    for (final path in _knownRootBinaries) {
      try {
        ...
        // Permission denied may occur on hardened kernels, which is safe
      }
    }

    // 2. Check Frida / dynamic hooks artifacts
    ...
    if (!isSafe) {
      AppLogger.warn('Security Audit Alert! Detected threats: $threats', tag: 'AntiTamper');
    } else {
      AppLogger.info('Device runtime integrity verified. Environment is clean.', tag: 'AntiTamper');
    }
  ...
  static Future<void> assertEnvironmentSafe() async {
    ...
    if (!status.isSecure) {
      throw SecurityException(
        'App execution prohibited on rooted/tampered devices: ${status.threatDetails.join("; ")}',
      );
    }
  }
}
