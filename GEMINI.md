...
24. **Quy chuẩn Tổ chức Thư mục `src/`, Zero-Hardcode, Đa ngôn ngữ (i18n), Zero Fake Default Values & Log Tiếng Anh:**
    * **Cấu trúc thư mục chuẩn `src/`:** 100% mã nguồn phát triển (`backend`, `mobile`, `frontend`) và kịch bản cơ sở dữ liệu (`db/migrations`) bắt buộc phải đặt bên trong thư mục [src/](file:///Users/micro/Source/erp/mifinace/src), tuyệt đối không tạo thư mục mã nguồn/CSDL rời rạc ngoài thư mục gốc.
    * **100% Enum-Driven & Zero-Hardcode:** Toàn bộ mã trạng thái (Status), phân loại (Type), vai trò (Role), phương thức (Method), nền tảng (Platform), mã lỗi (ErrorCode) bắt buộc phải được khai báo và sử dụng bằng Enum ở cả Backend và Frontend; cấm dùng chuỗi tự do (String literal) để so sánh hoặc gán logic.
    * **Zero Fake Default Values & No Mock Data:** Tuyệt đối cấm gán cứng các giá trị mặc định giả lập ngầm trong Entity, DTO, Request, Response (cấm `@Builder.Default` gán dữ liệu ngầm); dữ liệu bắt buộc lấy 100% từ request thực tế hoặc truy vấn CSDL thật (nếu chưa có thì để rỗng `null`/`""`/`[]`).
    * **100% i18n & Message Bundle:** Quản lý toàn bộ thông điệp qua hệ thống đa ngôn ngữ (`messages_*.properties` ở Backend và file JSON ở Frontend); Bean Validation dùng key template `{validation...}`; lỗi RFC 7807 phân giải qua `I18nService`.
    * **Zero Vietnamese in Technical Logs:** 100% thông điệp log (`log.info`, `log.warn`, `log.error`, `log.debug`) ở Backend, Frontend, Mobile bắt buộc viết bằng tiếng Anh kỹ thuật có cấu trúc, ngữ cảnh rõ ràng và đính kèm `traceId` (MDC). Tuyệt đối không log tiếng Việt.
